# サンプルアプリ一覧（AI受託開発エンジニア育成コース Ver2）

教材リポジトリ `ai-dev-school` の **`samples/`** に置く完成形・演習用コードのガイドです。
...
| **固定プロンプトの参照元** | 教材本文が指すプロンプト・ファイル構成の正 |
| **演習用の配布物** | 欠陥仕込み版など、コピーして使う題材 |
| **参照資料の置き場** | 議事録・要件定義テンプレートなど |

**`samples/` 自体をハンズオンの主作業場所にしないでください。**
...
| ファイル（例） | 使う節（例） |
| --- | --- |
| `20260708_…_議事録.md`・トランスクリプト | M3-2 ヒアリング |
...
1. **サンプルを直接編集しない** ── ハンズオンは受講生用ディレクトリで行う。サンプルはリポジトリの正本として残す。
...
6. **パス表記** ── 教材本文は `samples/todo-app/` など**サンプルへのパス**と、**受講生の作業パス**（`~/minutes-app` 等）を区別して書いています。迷ったら節本文の「準備」を正としてください。
...
- Phase 2：[`docs/policy/current/アプリ実装・AIコーディングの編集方針（Phase2）.md`](../docs/policy/current/アプリ実装・AIコーディングの編集方針（Phase2）.md)
- Phase 3：[`docs/policy/current/アプリ実装・AIコーディングの編集方針（Phase3）.md`](../docs/policy/current/アプリ実装・AIコーディングの編集方針（Phase3）.md)
- 節別の突合：[`docs/policy/current/教材整合性チェック方針.md`](../docs/policy/current/教材整合性チェック方針.md)

サンプルのファイル名・役割分担を変えたときは、**本文（`draft/ver2/`）と上記方針書**を同時に更新してください。
